...
void compress(std::array<std::uint32_t, 8>& h, const std::uint8_t block[64]) {
    std::uint32_t w[64];
    for (int i = 0; i < 16; ++i) {
...
std::string sha256_hex(const std::uint8_t* data, std::size_t len) {
    ...
    for (int j = 0; j < 8; ++j) block[63 - j] = std::uint8_t(bits >> (8 * j));
    compress(h, block);
    ...
    for (std::uint32_t v : h) {
        for (int shift = 28; shift >= 0; shift -= 4) out.push_back(hex[(v >> shift) & 0xf]);
    }
    return out;
}
